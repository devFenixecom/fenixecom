...
export function Slider() {
  const images = [
    {
      src: "https://www.apple.com/v/macbook-pro-13/o/images/overview/hero_endframe__bsza6x4fldiq_large_2x.jpg",
...
<div id="default-carousel" className="relative" data-carousel="static">
        {/* Carousel wrapper */}
        <div className="relative overflow-hidden rounded-lg md:h-120">
          {/* Item 1 
          <div className="z-95 text-3xl font-bold  mb-20 pt-40">
          {images[currentImageIndex].title}
            {images[currentImageIndex].text}</div>*/}
          <div className="duration-700 ease-in-out transition " data-carousel-item>
           
            <img src={images[currentImageIndex].src} className="z-60 block w-full " alt="..." />
          </div>
        ...
        {/* Slider indicators */}
        <div className="absolute z-90 flex space-x-3 -translate-x-1/2 bottom-5 left-1/2">
        ...
        {/* Slider controls */}
        <button type="button"
          onClick={handlePrevButtonClick} className="absolute top-0 left-0 z-30 flex items-center justify-center h-full px-4 cursor-pointer group focus:outline-none" data-carousel-prev>
        ...
        <button 
         ...
         type="button" className="absolute top-0 right-0 z-30 flex items-center justify-center h-full px-4 cursor-pointer group focus:outline-none" data-carousel-next>
          <span className="inline-flex items-center justify-center w-8 h-8 rounded-full sm:w-10 sm:h-10 bg-white/30 dark:bg-gray-800/30 group-hover:bg-white/50 dark:group-hover:bg-gray-800/60 group-focus:ring-4 group-focus:ring-white dark:group-focus:ring-gray-800/70 group-focus:outline-none">
