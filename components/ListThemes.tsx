...
export const ListThemes = () => {
  const { data, error } = useSWR('/api/themes', async (url) => {
    ...
    if (!data) return <div className="flex items-center justify-center w-full h-full">
...
<div className="w-full sm:w-2/3 px-4">
       ...
       {data.map((theme: any) => (
          <a href={`/theme/${theme.name}`} aria-label="View Item" key={theme._id}>
            <div className="relative overflow-hidden transition duration-200 transform rounded shadow-lg hover:-translate-y-2 hover:shadow-2xl">
              ...
              <div className="absolute inset-0 px-6 py-4 transition-opacity duration-200 bg-black bg-opacity-75 opacity-0 hover:opacity-100">
                <p className="mb-4 text-lg font-bold text-gray-100">{theme.name}</p>
